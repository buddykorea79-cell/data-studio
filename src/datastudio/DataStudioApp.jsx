...
export default function DataStudioApp({ onBack = null }){
  ...
  const handleFiles=useCallback(async(files)=>{
    ...
    if(results.length){
      ...
      setActiveTab("files");  // merge 대신 files 탭으로 — state 변경 최소화
    }
    setLoading(false);
  },[]);
  ...
  return<div style={{maxWidth:960,margin:"0 auto",padding:"1.5rem 1rem",fontFamily:"var(--font-sans)"}}>
    ...
    {/* Upload */}
    <div onDrop={onDrop} onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onClick={e=>{e.stopPropagation();inputRef.current?.click();}}
    ...
    <div style={{display:"flex",gap:0,marginBottom:18,borderBottom:`0.5px solid ${C.bd}`,overflowX:"auto"}}>
      {NAV_TABS.map(t=><button type="button" key={t.id} onClick={()=>!t.disabled&&setActiveTab(t.id)} disabled={t.disabled}
